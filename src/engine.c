...
void engine_run(engine* engine) {
    ...
    while (engine->running) {
        ...
        if (engine->_input.mouse_left_pressed) {
            ...
            if (clicked_idx == -1 && engine->arr_size < engine->max_rgbd) {
        ...
        if (engine->_input.mouse_left_released) {
            if (clicked_idx != -1) {
                engine->rgbd_array[clicked_idx].inv_mass = 1.0f / engine->rgbd_array[clicked_idx].mass;
            }
            clicked_idx = -1;
        }

        if (clicked_idx != -1) {
            engine->rgbd_array[clicked_idx].pos.x = engine->_input.mouse_x;
            engine->rgbd_array[clicked_idx].pos.y = engine->_input.mouse_y;
        }
